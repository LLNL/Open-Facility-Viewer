...
MACRO(LINK_WITH_VARIABLES TARGET_NAME)
    FOREACH(varname ${ARGN})
      IF (WIN32)
        IF(${varname}_RELEASE)
           IF(${varname}_DEBUG)
               TARGET_LINK_LIBRARIES(${TARGET_NAME} optimized "${${varname}_RELEASE}" debug "${${varname}_DEBUG}")
           ELSE(${varname}_DEBUG)
               TARGET_LINK_LIBRARIES(${TARGET_NAME} optimized "${${varname}_RELEASE}" debug "${${varname}_RELEASE}" )
           ENDIF(${varname}_DEBUG)
        ELSE(${varname}_RELEASE)
           IF(${varname}_DEBUG)
               TARGET_LINK_LIBRARIES(${TARGET_NAME} optimized "${${varname}}" debug "${${varname}_DEBUG}")
           ELSE(${varname}_DEBUG)
               TARGET_LINK_LIBRARIES(${TARGET_NAME} optimized "${${varname}}" debug "${${varname}}" )
           ENDIF(${varname}_DEBUG)
        ENDIF(${varname}_RELEASE)
      ELSE (WIN32)
        IF(${varname}_RELEASE)
           TARGET_LINK_LIBRARIES(${TARGET_NAME} "${${varname}_RELEASE}")
        ELSE(${varname}_RELEASE)
           TARGET_LINK_LIBRARIES(${TARGET_NAME} "${${varname}}")
        ENDIF(${varname}_RELEASE)
      ENDIF (WIN32)
    ENDFOREACH(varname)
ENDMACRO(LINK_WITH_VARIABLES TARGET_NAME)

MACRO(INSTALL_SCENEWORKS_TARGET TARGET_NAME RUNTIME_PATH)

# TRYME: Is CMAKE_CFG_INTDIR the same thing? Maybe not because of the "/" in the path?
IF(MSVC10)
	SET(VC_CONFIG_NAME "$(Configuration)")
ELSE(MSVC10)
	SET(VC_CONFIG_NAME "$(ConfigurationName)")
ENDIF(MSVC10)
...
IF( MSVC )
ADD_CUSTOM_COMMAND(TARGET ${TARGET_NAME} POST_BUILD COMMAND ${CMAKE_COMMAND}  -DBUILD_TYPE=${VC_CONFIG_NAME} -P cmake_install.cmake)
ENDIF( MSVC )

ENDMACRO(INSTALL_SCENEWORKS_TARGET TARGET_NAME)

MACRO(SET_MSVC_TARGET TARGET_NAME OUTPUT_DIR)
if ( MSVC )
	SET_TARGET_PROPERTIES(${TARGET_NAME} PROPERTIES DEBUG_POSTFIX "${CMAKE_DEBUG_POSTFIX}")	
endif ( MSVC )

INSTALL_SCENEWORKS_TARGET(${TARGET_NAME} "bin")

ENDMACRO(SET_MSVC_TARGET TARGET_NAME OUTPUT_DIR)